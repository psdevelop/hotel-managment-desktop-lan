  ...
  if BoldHandlesDM.blhAllRooms.Count>0 then
     begin
       BoldHandlesDM.blhAllRooms.First;
         while True do
           begin
             ssRoomsAllocationChart.AddResource('Номер '+
...
begin
  if BoldHandlesDM.blhAllRooms.Count>0 then
    ...
    ShowMessage('В системе не учитывается ни одного номера!');
end;
    ...
    if BoldHandlesDM.blhClientResCountByRoom.Count=0 then
      begin
        ...
        if BoldHandlesDM.blhAllReserved.List.IndexOf(
          ...
          ShowMessage('Снимите фильтры в таблице бронирования для выделения позиции запрашиваемой брони!');
          end
    else
      ShowMessage('У данного клиента по данному номеру есть непогашеная бронь!');
...
begin
  ...
  if BoldHandlesDM.blhLastRoomReserve.List.Count>0 then
     ShowMessage('По выделенному номеру есть непогашенная бронь!');
  ...
  if BoldHandlesDM.blhClientSearch.Count>0 then
   ...
   ShowMessage('Не выделено ни одного клиента в таблице бронирования!');
        ...
        if MainLeftBoldTreeView.Selected.Follower.Element.ClassType.ClassName='TNomer_arendy' then
            ...
            begin
              ...
              if BoldHandlesDM.blhAllReserved.List.IndexOf(
                               ...
                               ShowMessage('Снимите фильтры с таблицы бронирования, невозможно выделить добавленное бронирование!');
            ...
            begin
              ...
              ShowMessage('Снимите фильтры с таблицы ожидания, невозможно выделить добавленное ожидание!');
...
begin
if MessageDlg('После нажатия кнопки текущая стоимость будет внесена как базовая для расчёта процентов водителю, продолжить?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
   begin
     ...
     (BoldOthHandleCompDM.brhCurrClientDoc.Value as TList_klienta).Naimenovanie:=
       'Расход на проценты водителю, аренда номера '+
       IntToStr((BoldOthHandleCompDM.brhCurrClientDoc.Value as TList_klienta).Opis_progiv_v.Nomer)+
       ', дата вселения - '+DateTimeToStr((BoldOthHandleCompDM.brhCurrClientDoc.Value as TList_klienta).Data_vjezda);
...
BoldOthHandleCompDM.brhCurrClient.Value:=
 ...
 if MainLeftBoldTreeView.GetElementAt(X,Y)<>nil then
          //Если бросают на Класс
          if MainLeftBoldTreeView.GetElementAt(X,Y).ClassType.ClassName='TKlass_nomera' then
            begin
               //Если бросают с Таблицы
               if (Source is TBoldGrid) then
               begin

               end
               //Если бросают с Дерева
               //Если бросают Номер на Класс с Дерева
               else
               begin
                if RoomTreeBE<>nil then
                 begin
                  if MessageDlg('Привязать к классу номеров?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
                    begin
        ...
        //Если бросают на номер
            ...
            begin
              ...
              if (Source is TBoldGrid) then
               begin
                 //Если бросают Клиента
                 if DragClientBE<>nil then
                   begin

                         if MessageDlg('Забронировать номер на выделенного клиента?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
                           begin
                             AddResToRoom((DragClientBE as TKlient),
                               (BoldOthHandleCompDM.brhCurrRoom.Value as TNomer_arendy));
                           end
                         else if MessageDlg('Установить ожидание номера на выделенного клиента?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
                           begin
               ...
               //Если бросают не с таблицы
               else
                 begin
                   //Если бросают ожидание
                   ...
                   //Если бросают бронь
                     ...
                     begin
                       ...
                       begin
                         ShowMessage('Клиент совершил предоплату, она будет добавлена к оплате за проживание!');
                         SBE:=
            ...
            begin
              if (Source is TBoldGrid) then
               begin
                 if DragClientBE<>nil then
                   begin
                     if MessageDlg('Добавить клиента в лист проживания?', mtConfirmation,[mbYes,mbNo],0) = mrYes then
                       begin
  ...
  if SetDTForm.ShowModal=mrOk then
    begin
     ...
     ((BoldOthHandleCompDM.brhCurrClientDoc.Value as TList_klienta).Data_vyezda<StrToDate('01.01.2000')) then
      ...
      ApartmInfo+', дата въезда '+DateTimeToStr((BoldOthHandleCompDM.brhCurrClientDoc.Value
        as TList_klienta).Data_vjezda)+', дата выезда '+DateTimeToStr((BoldOthHandleCompDM.brhCurrClientDoc.Value
        as TList_klienta).Data_vyezda));
      ...
      ShowMessage('Дата начала должна быть меньше, чем дата конца!');   
    end;
end;
  ...
  if SetDTForm.ShowModal=mrOk then
    begin
      ...
      ApartmInfo+', дата въезда '+DateTimeToStr((BoldOthHandleCompDM.brhCurrClientDoc.Value
       ...
       ShowMessage('Конечная дата должна быть больше чем начальная!');     
    end;
end;
...
if Button = mbLeft then
  ...
  ShowMessage('Отсутствуют строки, операции недоступны!');
end;
end;
